...
namespace gl
{
	...
	void Shape::Rotate(float _x, float _y, float _z)
	{
		SetRotation(rot[0] + _x, rot[1] + _y, rot[2] + _z);
	}

	void Shape::CalculateOffset()
	{
	...
	void Cube::SetPosition(float _x, float _y, float _z, bool _setAnchor/* = true*/)
	{
		if (_setAnchor)
			SetAnchor(_x, _y, _z);
	...
	void Cube::Move(float _x, float _y, float _z, bool _moveAnchor/* = true*/)
	{
		if (_moveAnchor)
		{
			MoveAnchor(_x, _y, _z);
